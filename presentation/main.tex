...
\begin{frame}
  ...
  \note{
    \begin{itemize}
      \item In this presentation the main topics of the paper are presented, in the same order they appear in the paper.
  ...
  \begin{itemize}
    ...
    \item<3-> Prediction errors:
      \begin{align}
        ...
        \quad .
      \end{align}
    \note[item]<3->{Prediction errors can be introduced as new variables to extend the dynamical system and to allow learning.}
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \note[item]<1->{Model parameters are variables mean and variance.}
    \item<2-> Equivalent to maximize negative free energy with respect to parameters.
    \note[item]<2->{Feature and stimulus joint probability, hence free energy, is maximized instead of the marginal likelihood.}
    % MC continue with a better explanation to merge convergence of sample mean and gradients written with prediction errors.
    \note[item]<3->{Parameters are updated each time with different samples hence the convergence is guaranteed only as mean over all samples.}
    \note[item]<3->{Introducing prediction errors as variables of the model allows to learn model parameters.}
  \end{itemize}
\end{frame}
