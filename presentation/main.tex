...
\begin{frame}
  ...
  \note{
    \begin{itemize}
      \item Note that all three hypotheses are satisfied.
    \end{itemize}
  }
\end{frame}
  ...
  \begin{itemize}
    ...
    \item<2-> Equivalent to maximize negative free energy with respect to parameters:
      \begin{align}
        ...
        \quad .
      \end{align}
    \note[item]<2->{The fixed point of this dynamical system exists only as sample mean over the occured events of perception.}
...
\begin{frame}
  ...
  \note{
    \begin{itemize}
      ...
      \item The inverse of covariance matrix depends on non-adjacent neurons, Hebbian plasticity is partially satisfied due to the nonlinear relation.
    \end{itemize}
  }
\end{frame}
  ...
  \begin{itemize}
    ...
    \item<2-> Generalized equations for the inference task:
      \begin{align}
        ...
        \dot{\vec{\phi}}_i & = - \vec{\varepsilon}_i + h^\prime(\vec{\phi}_i) \times \Theta_{i-1}^\transpose \vec{\varepsilon}_{i-1}
        \quad , \\
        \label{eq:dynamical_system_epsilon}
        \dot{\vec{\varepsilon}}_i & = \vec{\phi}_i - \Theta_i h(\vec{\phi}_{i+1}) - \Sigma_i \vec{\varepsilon}_i
        \quad .
      \end{align}
    \item<2-> Generalized equations for the learning task:
      \begin{align}
        \label{eq:dynamical_system_sigma}
        \pdv{F}{\Sigma_i} & = \frac{1}{2} (\vec{\varepsilon}_i \vec{\varepsilon}_i^\transpose - \Sigma_i^{-1})
        \quad , \\
        \label{eq:dynamical_system_theta}
        \pdv{F}{\Theta_i} & = \vec{\varepsilon}_i h(\vec{\phi}_{i+1})^\transpose
        \quad .
      \end{align}
